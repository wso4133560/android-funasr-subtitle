...
static void trace_stage(const char *, ...) {}
...
struct model { cfg c; gguf_context*gguf=nullptr; ggml_context*ctx_meta=nullptr; ggml_context*ctx_w=nullptr;
  ...
  ggml_tensor* g(const std::string&n){auto it=t.find(n);if(it==t.end()){fprintf(stderr,"missing %s\n",n.c_str());throw std::runtime_error("SenseVoice native initialization failure");}return it->second;} };
...
struct SenseVoice::Impl {
  ...
  {int ki=gguf_find_key(gg,"sv.vocab"); if(ki>=0){int nv=gguf_get_arr_n(gg,ki); vocab.resize(nv); for(int i=0;i<nv;i++){const char*s=gguf_get_arr_str(gg,ki,i); vocab[i]=s?s:"";}}}
  ...
  D=m.c.d_model; V=m.c.vocab;
  if (vocab.empty()) throw std::runtime_error("Missing SenseVoice vocabulary");   // fall back to ids if the gguf has no vocab
  ...
  if(embed->ne[0]!=F){fprintf(stderr,"unexpected embed width: %lld\n",(long long)embed->ne[0]);throw std::runtime_error("Invalid SenseVoice model");}
  for(int id:qtok) if(id<0||id>=embed->ne[1]){fprintf(stderr,"query token out of embed range: %d\n",id);throw std::runtime_error("Invalid SenseVoice model");}
  embed_f32.resize((size_t)ggml_nelements(embed));
  if(embed->type==GGML_TYPE_F32){
    ...
    fprintf(stderr,"unsupported embed type: %s\n",ggml_type_name(embed->type));throw std::runtime_error("Invalid SenseVoice model");
  }

  }
  std::string run(const std::vector<float>& fb,int T){
    ...
    ggml_init_params cp={(size_t)64*1024*1024,nullptr,true}; ggml_context*c=ggml_init(cp);
    if(!c) throw std::runtime_error("Graph metadata allocation failed");
    ...
    // CTC can emit a transient <|nospeech|> meta token inside an otherwise
    // valid utterance; detok_sv() removes meta tokens from the text.
    std::string result;
    result=detok_sv(seg_ids,vocab,false);
    // The VAD has already gated this window. Some CTC paths omit the Speech
    // meta tag while still producing lexical tokens, so non-empty text is the
    // reliable final criterion here.
    return result;
